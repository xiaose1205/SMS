namespace HelloData.FrameWork.Text
{
    using System;
    using System.IO;
    using System.Text;
        ...
        internal static string[] nicename = new string[]
   {
    ...
    , "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER"
        ...
        {
            List<byte> int13 = new List<byte>();
            List<byte> int10 = new List<byte>();
            List<byte> firstLineBytes = new List<byte>();
            FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read);
            byte[] buffer = new byte[fs.Length];
            fs.Read(buffer, 0, buffer.Length);
            List<byte> lineBytes = firstLineBytes;
            foreach (var item in buffer)
            {
                ...
                if (int13.Count == 1 && int10.Count == 1)
                {
                    break;
                }
            ...
            string encodeingName = GetEncodingName(ToSByteArray(firstLineBytes.ToArray()));

            if (encodeingName.ToUpper() == "OTHER")
            {
                throw new EncodingException(" Encodeing Name is Other");
            }

            return Encoding.GetEncoding(encodeingName);
        }
        #endregion
